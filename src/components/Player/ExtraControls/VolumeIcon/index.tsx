...
import { getConfigClient, setMuted } from 'store/slices/configClientSlice'

import { VolumeIconStyled } from './styles'

export const VolumeIcon: React.FC = () => {
  const dispatch = useAppDispatch()
  const { muted, volume } = useAppSelector(state => getConfigClient(state))
  ...
  return (
    <>
      <VolumeIconStyled onClick={() => dispatch(setMuted(!muted))}>
        <svg
          role="presentation"
          height="16"
          width="16"
